...
class Java_Class:
    ...
    def add_constant_raw(self, data):
        data.link(self.const_pool)
    ...
    def add_interface_raw(self, class_ref):
        self.interfaces.append(class_ref)

    def add_field_raw(self, field_info):
        field_info.link(self.const_pool)
        self.fields.append(field_info)

    def add_method_raw(self, method_info):
        method_info.link(self.const_pool)
    ...
    def from_bytes(input_bytes: bytes):
        ...
        def read_struct(fmt):
            ...
            values = struct.unpack(fmt, input_bytes[cursor:cursor + chunk_size])
            cursor += chunk_size
            return values
        ...
        for _ in range(methods_count):
            ...
            for _ in range(attributes_count):
                (attribute_name_index, attribute_length) = read_struct(">HL")
                (info, ) = read_struct(f"{attribute_length}s")
                method.add_attribute_raw(Attribute_Info(attribute_name_index,
                                                        info))
        ...
        for _ in range(attributes_count):
            (attribute_name_index, attribute_length) = read_struct(">HL")
            (info, ) = read_struct(f"{attribute_length}s")
            out_java.add_attribute_raw(Attribute_Info(attribute_name_index, info))
        return out_java